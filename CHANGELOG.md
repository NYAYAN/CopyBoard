...
## 🔍 Karşılaştırma
- **Galeriden iki ila beş görüntüyü yan yana koyup karşılaştırma.** Büyük görüntüleyicinin araç çubuğuna `Karşılaştır` geldi: basınca alt şerit seçim kipine giriyor, seçilenler sahnenin yerini alan bir ızgarada açılıyor. Sahne bir resim taşımak için var; birkaç tanesi aynı anda ancak onun yerine geçen bir ızgarada durabilir.
- **Seçim bir gezinme kipi, tıklama listeye atma değil.** Şeritte bir küçük resme tıklamak onu sahnede büyük açıyor — `←`/`→`, `‹ ›` okları, yakınlaştırma ve resmin ölçü/ağırlık/tarih çipleri seçim boyunca duruyor. 68px'lik bir küçük resimden karşılaştırılacak resim seçilemez. Karşılaştırmaya almak ayrı ve açık bir eylem: sahnedeki resim için alt çubuktaki `Karşılaştırmaya ekle` düğmesi (zaten ekliyse `Karşılaştırmadan çıkar` olup renk değiştiriyor, yani baktığınız resmin ızgarada olup olmadığını okuyabiliyorsunuz), ya da bulunduğunuz resimden ayrılmadan eklemek için küçük resmin köşesindeki `+`. O `+`, eklendiğinde ızgaradaki sıra numarasına dönüşüyor: aynı yerde duran tek kontrol, hem ekleme hem sıra göstergesi. Şeritte iki durum iki ayrı kanalda: **ızgarada** olan accent kenarlık + numaralı çip, **ekranda** olan küçük resmin çevresinde bir halka — biri diğerini gizlemiyor.
- **Üç yerleşim:** hepsi `Yatay` tek sırada (varsayılan), `2'li` ve `4'lü`. Beş panel varsayılan pencere genişliğinde yan yana sığıyor — ölçüldü: 1632px pencerede panel başına 316px, yatay kaydırma yok. Panel başına 300px taban var, o yüzden elle daraltılmış bir pencerede ızgara kaydırılıyor; 250px'lik bir panelde karşılaştırılacak bir şey kalmıyor. Yerleşim değiştirmek panellerin ölçeğini bozmuyor.
- **Panelleri başlığından sürükleyerek sıralama.** Sıralama panel düğümlerini TAŞIYOR, yeniden çizmiyor: yeniden çizmek beş tam çözünürlüklü resmi baştan çözerdi. Sıra tek yerde tutuluyor ve iki yerde görünüyor — panel başlıklarındaki numaralar ve şeritteki çipler aynı anda güncelleniyor. HTML5 sürükle-bırak değil, pointer olayları: paneller çok megabaytlık data URL taşıyor ve yerel sürükleme, platformdan onlardan biriyle sürükleme görseli üretmesini isterdi.
- **Yakınlaştırma her panel için ayrı.** Bir resme tıklamak (sığdır ↔ %100) ya da üzerinde `Ctrl/Cmd`+tekerlek çevirmek — trackpad'de iki parmakla sıkıştırma aynı olay olarak geliyor — yalnızca o paneli ölçekliyor. Ayrı resimler bunlar; birine yakınlaşmak hepsini yakınlaştırmamalı. İmlecin altındaki nokta yerinde kalıyor (ölçüldü: tekerlek öncesi ve sonrası imlecin altındaki resim pikseli aynı), imlecin olmadığı paneller kendi ortasını koruyor — o ikinci yarı olmasa toplu bir ölçek değişimi her paneli kendi sol üst köşesine kaçırırdı.
- **Hepsini birlikte ölçeklemek ayrı bir eylem:** araç çubuğundaki yakınlaştırma kontrolü ve `Ctrl/Cmd ±` `0` `1` bütün panelleri aynı ölçeğe getiriyor. Kapsam karışmasın diye o düğmelerin ipuçları karşılaştırmada "Tümünü yakınlaştır / Tümünü uzaklaştır / Tümü: gerçek boyut" oluyor. Adım referansı 1 numaralı panel, yani paneller farklı ölçeklerdeyken `+` hepsini tek ölçekte topluyor. Okuma alanı üç durumu ayırıyor: hepsi sığdırılmışsa `Sığdır` (ızgarada sığdır her panelde farklı bir yüzde, yazılacak tek dürüst sayı yok), hepsi aynı ölçekteyse yüzde, farklıysa `Karışık`.
- **Karşılaştırma açıkken şerit canlı kalıyor:** ızgaradan çıkmadan resim ekleyip çıkarılabiliyor. Bir resim galeriden silinirse ızgaradaki paneli de gidiyor; seçim ikinin altına düşerse tek başına kalan bir panel bırakmak yerine seçim kipine dönülüyor. Panel ile yakınlaştırması her zaman birlikte düşüyor (panelin `×`'i, şeritten seçimi kaldırmak, galeriden silmek — üçü de tek yoldan geçiyor), yoksa çıkarılan bir resim geri eklendiğinde eski yakınlaştırmasıyla geliyordu.
- Paneller **tam çözünürlüklü** resmi gösteriyor, şeridin 360px'lik küçük resmini değil: 360px'te yapılan karşılaştırma hiçbir soruyu cevaplamaz. Tavan da bu yüzden var — en fazla beş resim. Çözülmüş bir 2560×1600 yakalama, PNG'si kaç kilobayt olursa olsun ~16MB bitmap, ve hepsi aynı anda açık. Altıncıyı eklemeye çalışınca alt çubuk sınırı söylüyor; karşılaştırmada olmayan bir resme geçtiğinizde ekleme düğmesi baştan devre dışı, yani sınırı tıklamadan önce görüyorsunuz.

## 🪟 Pencere düğmeleri
- **Çerçevesiz görüntüleyici penceresinin küçültme ve tam ekran düğmesi yoktu**, yalnızca kapatma vardı; pencere çerçevesiz olduğu için o düğmeler bize ait ve hiç çizilmemişlerdi. İkisi de `×`'in soluna geldi. Tam ekran düğmesi durumu kendi içinde saymıyor, pencereye soruyor: başlık çubuğuna çift tıklamak ve yapıştırma jestleri de pencereyi büyütüyor, o yollarla büyüdüğünde de simge "önceki boyut"a dönüyor ve ipucu yazısı değişiyor.
- **Büyütülen pencerenin kenarları ekranın dışında kalıyordu.** Windows, çerçevesiz bir pencereyi büyütürken kendi yeniden boyutlandırma kenarlığını pay olarak veriyor: pencere dikdörtgeni çalışma alanının her yanından 8px dışına taşıyor (ölçüldü, %100 ölçekte `-8,-8 +16x16`). Çerçeveli bir pencerede o payı görünmez çerçeve yutuyor; çerçevesiz olanda içerik doğrudan orada, dolayısıyla araç çubuğunun üst satırı, kapatma düğmesinin sağ kenarı ve alt şeridin alt kenarı ekran dışında çiziliyordu — alt kenar görev çubuğunun arkasında. Dikdörtgen düzeltilebilecek bir şey değil: Chromium o pencere stilini gölge ve yapıştırma jestleri için tutuyor, ve pencere büyükken `setBounds()` yoksayılıyor (ölçüldü). Bunun yerine **içerik** taşma kadar içeri alınıyor — sabit bir 8 değil, ana süreçte ölçülen değer, çünkü pay DPI ile değişiyor — ve pencere küçültülünce sıfıra dönüyor.
- Görüntüleyici penceresinin **görev çubuğu düğmesi artık uygulamanın kendi simgesini** taşıyor. Uygulamanın diğer bütün pencereleri `skipTaskbar`, yani görev çubuğunda görünen tek pencere bu; simgesini Windows'un çalıştırılabilir dosyadan çıkarmasına bırakmak için sebep yok.

## 🎨 Küçük düzeltmeler
- Karşılaştırma kipinde **araç çubuğu düğmeleri pencerenin sol üstüne kayıyordu.** Başlık çipleri gizlendiğinde `space-between` yerleştirecek tek çocuk buluyor ve onu satırın BAŞINA alıyor; pencere düğmeleri de o grubun içinde. Kip değişince yer değiştiren bir tam ekran düğmesi, aradığınız düğme olur. Beş durumda ölçüldü: grup sağ kenardan hep 10px boşlukla bitiyor, 620px'lik dar pencerede de taşan düğme yok.
- Koyu temada şeritteki **seçim numarası çipi 4.23:1** ölçüyordu, 10px bir etiket için bu projenin tuttuğu 4.5:1'in altında — danger düğmelerini `--danger-solid`'e taşıyan ölçümün aynısı. Çipin zemini accent'ten bir tık koyu: 5.44:1 koyu, 7.13:1 açık. Yarı saydam zeminleri düz renk sayan bir ölçüm ilk turda her şeyi "1.19:1" gösterdiği için ölçüm de düzeltildi: arka planlar üst üste bindirilerek hesaplanıyor.
- Seçim kipinde eklenmemiş küçük resimler %30 opaklığa düşüyordu ve elenmiş gibi duruyordu; artık şeridin normal %55'inde — hâlâ gidip bakılacak şeyler.
