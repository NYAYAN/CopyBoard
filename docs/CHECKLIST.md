...
- [~] A14. Video: ~1 dk kayıt → Durdur → kayıt duruyor ama KAYDETME PANELİ gelmiyor (2026-09-04, diğer PC, 3 monitör). KÖK NEDEN: `record_stop` paneli açmadan ÖNCE kaydedici overlay'ini gizliyor ve panele parent VERMİYORDU; sahipsiz panel Windows'ta öndeki pencereye/birincil monitöre göre konumlanıyor, üç monitörde kullanıcının baktığı ekranda açılmıyordu. Ekran görüntüsü yolu (`capture::save_png`) bunu zaten doğru yapıyordu (gizleme yok, yalnız her-zaman-üstte indir + `set_parent`). Düzeltme: aynı kalıp kayda uygulandı; ayrıca var olmayan Videolar dizini verilmiyor ve tüm dallar günlüğe yazılıyor. Doğrulama: `--qa` 9f — Durdur → panel açıldı, ÖNDE, kaydın yapıldığı monitörde, iptalde yol panoya. 3 monitörde gözle bekliyor
